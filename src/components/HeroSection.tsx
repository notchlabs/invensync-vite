...
import { ArrowRight, CheckCircle, Search, Filter, Plus, ScanLine, Edit2 } from 'lucide-react'
...
/* ===== Browser Mockup — intentionally always light to simulate real browser UI ===== */
  ...
  return (
    <div className="bg-app rounded-2xl overflow-hidden border border-border-main hover:-translate-y-1 transition-all duration-400">
      ...
      <div className="flex items-center justify-between px-4 py-2.5 bg-header border-b border-border-main">
        ...
        <div className="inline-flex px-10 py-1 bg-surface border border-border-main rounded-md text-[11px] text-muted-text font-body">
          app.invensync.in
        </div>
        <div className="w-14" />
      </div>
      ...
      {/* Body */}
      <div className="p-5 text-secondary-text font-body max-h-[400px] overflow-hidden relative after:absolute after:bottom-0 after:left-0 after:right-0 after:h-[70px] after:bg-gradient-to-t after:from-app after:to-transparent after:pointer-events-none">
        <div className="flex items-start justify-between mb-3.5">
          <div>
            <h3 className="text-[17px] font-bold text-primary-text font-display mb-0.5">Inventory Management</h3>
            <p className="text-[11px] text-muted-text">Track and manage your stock across all sites</p>
          </div>
          <div className="flex gap-1.5">
            <button className="inline-flex items-center gap-1 px-3 py-1.5 text-[10px] font-semibold bg-surface text-secondary-text border border-border-main rounded-md cursor-default"><ScanLine size={11} /> Consumption</button>
        ...
        <div className="flex gap-1.5 mb-2.5">
          <div className="flex items-center gap-1.5 px-2.5 py-1.5 bg-surface border border-border-main rounded-md text-[10px] text-muted-text min-w-[140px]"><Search size={11} /><span>Search...</span></div>
          <div className="flex items-center gap-1 px-2.5 py-1.5 bg-surface border border-border-main rounded-md text-[10px] text-muted-text"><Filter size={9} /> Select Site(s)</div>
          <div className="flex items-center gap-1 px-2.5 py-1.5 bg-surface border border-border-main rounded-md text-[10px] text-muted-text"><Filter size={9} /> Select Vendor(s)</div>
        </div>
        <p className="text-[10px] text-muted-text mb-2">Showing <strong className="text-primary-text">10</strong> of <strong className="text-primary-text">2,939</strong> available products</p>
        <div className="border border-border-main rounded-lg overflow-hidden overflow-x-auto">
          <div className="min-w-[600px]">
            <div className="flex px-3 py-2 gap-1.5 text-[9px] font-bold uppercase tracking-wide text-muted-text bg-surface border-b border-border-main">
              <span className="w-7 flex items-center"><span className="w-3.5 h-3.5 border-[1.5px] border-border-main rounded-sm" /></span>
            ...
            {products.map((p, i) => (
              <div key={i} className="flex px-3 py-2.5 gap-1.5 text-[10px] text-secondary-text border-b border-border-main/50 last:border-b-0 items-center hover:bg-surface transition-colors">
                <span className="w-7 flex items-center"><span className="w-3.5 h-3.5 border-[1.5px] border-border-main rounded-sm" /></span>
                <span className="flex-[2.2] flex items-center gap-2 min-w-0">
                  <span className="w-[30px] h-[30px] bg-header border border-border-main rounded-[5px] shrink-0" />
                  <span className="min-w-0">
                    <span className="flex items-center gap-1 font-semibold text-primary-text text-[11px] whitespace-nowrap overflow-hidden text-ellipsis">{p.name} <Edit2 size={9} className="text-muted-text" /></span>
                    <span className="block text-[9px] text-muted-text">{p.vendor}</span>
                  </span>
                </span>
                <span className="flex-1 text-secondary-text truncate">{p.site}</span>
                ...
                <span className="flex-[0.8] text-right font-semibold text-primary-text">{p.price}</span>
                <span className="flex-[0.9] text-right font-bold text-primary-text">{p.total}</span>
              </div>
            ))}
          </div>
        </div>
...
const HeroSection = () => {
  ...
  return (
    <section ref={heroRef} className="relative flex flex-col items-center overflow-hidden pt-[140px] pb-20 bg-card" id="hero">
      {/* Ambient glows */}
      ...
      <div className="relative z-10 text-center flex flex-col items-center gap-5 mb-16 max-w-[1200px] mx-auto px-6">
        <div className="inline-flex items-center gap-2 px-[18px] py-[7px] text-[13px] font-semibold text-secondary-text bg-surface border border-border-main rounded-full tracking-wide animate-[fadeInDown_0.8s_ease]">
          <span className="w-[7px] h-[7px] bg-blue-600 rounded-full animate-[pulse-dot_2s_ease-in-out_infinite]" />
          Built for Indian Businesses
        </div>

        <h1 className="text-[64px] max-lg:text-[50px] max-md:text-[38px] font-bold tracking-[-2.5px] max-md:tracking-[-1.5px] leading-[1.05] text-primary-text max-w-[800px] animate-[fadeInUp_0.8s_ease_0.1s_both] font-display">
          Your Bills, Stock &amp; Profits<br />
          <span className="text-muted-text">All in One Place.</span>
        </h1>

        <p className="text-[18px] max-md:text-[16px] leading-relaxed text-secondary-text max-w-[600px] animate-[fadeInUp_0.8s_ease_0.25s_both]">
        ...
        <div className="flex flex-wrap justify-center gap-6 mt-3 animate-[fadeInUp_0.8s_ease_0.55s_both] max-md:flex-col max-md:items-center max-md:gap-3">
          {features.map((f, i) => (
            <span key={i} className="inline-flex items-center gap-1.5 text-[14px] text-secondary-text font-medium">
              <CheckCircle size={16} strokeWidth={2.5} className="text-muted-text" />
              {f}
            </span>
          ))}
        </div>
