  ...
  return (
    <div className="bg-[#fafafa] rounded-2xl overflow-hidden border border-neutral-200 hover:-translate-y-1 transition-all duration-400">
      <div className="flex items-center justify-between px-4 py-2.5 bg-[#e8e8e8] border-b border-[#ddd]">
        ...
        <span className="px-10 py-1 bg-[#f5f5f5] border border-[#ddd] rounded-md text-[11px] text-neutral-500">app.invensync.in/sites</span>
        <div className="w-14" />
      </div>
      <div className="p-5 font-body max-h-[360px] overflow-hidden overflow-y-auto custom-scrollbar relative after:absolute after:bottom-0 after:left-0 after:right-0 after:h-16 after:bg-gradient-to-t after:from-[#fafafa] after:to-transparent">
        <div className="flex items-start justify-between mb-3">
          <div><h3 className="text-[17px] font-bold text-neutral-900 font-display">All Sites</h3><p className="text-[11px] text-neutral-400">Manage and monitor all your sites</p></div>
        ...
        <div className="grid grid-cols-2 max-sm:grid-cols-1 gap-3 pb-8">
          {sites.map((s, i) => (
            <div key={i} className="bg-white border border-neutral-200 rounded-xl p-3.5 hover:-rotate-1 hover:border-neutral-300 transition-all duration-300">
              <div className="flex justify-between items-center mb-2">
                <strong className="text-[13px] text-neutral-900 truncate pr-2">{s.name}</strong>
                <span className="w-7 h-7 shrink-0 flex items-center justify-center border border-neutral-200 rounded-md"><Edit2 size={10} className="text-neutral-400" /></span>
              </div>
              <div className="flex flex-col gap-0.5 text-[10px] text-neutral-500 mb-3">
              ...
              <div className="flex flex-col sm:flex-row justify-between items-start sm:items-end gap-2">
                ...
                <span className="text-left sm:text-right text-[10px] text-neutral-400">Inventory Value<br /><strong className="text-[13px] text-neutral-900">{s.val}</strong></span>
              </div>
            </div>
          ))}
        </div>
