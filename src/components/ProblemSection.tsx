...
const ProblemSection = () => {
  ...
  return (
    <section className="relative py-28 max-md:py-16 overflow-hidden bg-app border-t border-b border-border-main" id="problem">
      ...
      <div className="text-center mb-20 relative z-[1] max-w-[1200px] mx-auto px-6">
        <span className="inline-block text-[12px] font-bold tracking-[2.5px] uppercase text-secondary-text mb-4 px-[18px] py-1.5 bg-card border border-border-main rounded-full">
          The Problem
        </span>
        <h2 className="text-[48px] max-md:text-[34px] font-bold text-primary-text mb-4 tracking-tight font-display">
          A Day <span className="text-muted-text">Without</span> InvenSync
        </h2>
      ...
      <div className="max-w-[1200px] mx-auto px-6 mb-28 max-md:mb-16 relative z-[1]" ref={timelineRef}>
        ...
        <div className="grid grid-cols-[140px_1fr_1fr] max-lg:hidden gap-[4px] mb-[4px]">
          <div />
          <div className="flex items-center gap-2 px-5 py-3 text-[13px] font-bold uppercase tracking-wide text-secondary-text bg-card border border-border-main border-b-0 rounded-t-lg">
            <X size={14} strokeWidth={3} /> Manual Process
          </div>
          <div className="flex items-center gap-2 px-5 py-3 text-[13px] font-bold uppercase tracking-wide text-primary-text bg-header border border-border-main border-b-0 rounded-t-lg">
        ...
        {timelineData.map((row, i) => (
          ...
          >
            <div className="flex flex-col max-lg:flex-row items-start max-lg:items-center justify-center px-3 py-5 gap-1 max-lg:gap-2.5 max-lg:px-4 max-lg:py-3">
              <span className="font-display text-[13px] font-bold text-primary-text bg-card border border-border-main px-2.5 py-1 rounded-md tracking-wide">{row.time}</span>
              <span className="text-[11px] text-muted-text font-medium uppercase tracking-wide">{row.title}</span>
            </div>
            <div className="flex items-start gap-4 px-6 py-6 bg-card border border-border-main border-l-[3px] border-l-red-400 rounded-md text-sm text-secondary-text leading-relaxed">
              <span className="shrink-0 w-6 h-6 flex items-center justify-center rounded-full bg-red-50 text-red-500 mt-0.5">
                <X size={12} strokeWidth={3} />
              </span>
              <span>{row.manual}</span>
            </div>
            <div className="flex items-start gap-4 px-6 py-6 bg-surface border border-border-main border-l-[3px] border-l-blue-500 rounded-md text-sm text-primary-text leading-relaxed">
      ...
      <div className="max-w-[1200px] mx-auto px-6 relative z-[1]">
        <div className="text-center mb-16">
          <span className="inline-block text-[12px] font-bold tracking-[2.5px] uppercase text-muted-text mb-2">The Reality</span>
          <h3 className="text-[40px] max-md:text-[28px] font-bold text-primary-text tracking-tight font-display">Sound Familiar?</h3>
        </div>
        <div className="grid grid-cols-2 max-md:grid-cols-1 gap-6">
          {painPoints.map((card, i) => (
            <div
              ...
              data-card-index={i}
              className={`flex items-start gap-5 p-8 max-md:p-6 bg-card border border-border-main border-l-4 border-l-blue-500 rounded-2xl hover:bg-surface hover:-translate-y-1 hover:border-blue-600 transition-all duration-700 ease-[cubic-bezier(0.16,1,0.3,1)] ${
            ...
            >
              ...
              <div>
                <h4 className="text-[16px] font-bold text-primary-text mb-2 leading-snug italic">{card.title}</h4>
                <p className="text-[14px] text-secondary-text leading-relaxed">{card.desc}</p>
              </div>
            </div>
          ))}
        </div>
