...
const useCases = [
  {
    ...
    accentClass: 'bg-app',
    tabClass: 'bg-app',
    activeTabClass: 'bg-card border-stone-200 text-primary-text border-b-2 border-b-stone-800',
    ...
    Visual: () => (
      <div className="w-full h-full min-h-[220px] sm:min-h-[300px] flex items-center justify-center p-4 sm:p-8 relative overflow-hidden">
        ...
        <div className="relative z-10 flex flex-col gap-4 sm:gap-6 w-full max-w-[280px]">
          <div className="bg-card border border-stone-200 rounded-lg p-3 sm:p-4 rotate-[-2deg] flex gap-3 items-center">
            ...
            <div><div className="text-[9px] sm:text-[10px] font-bold uppercase">Alert</div><div className="text-[11px] sm:text-[13px] font-bold ">Cement BOQ @ 85%</div></div>
          </div>
          <div className="bg-card border border-stone-200 rounded-lg p-3 sm:p-4 rotate-[1deg] ml-8 flex gap-3 items-center">
            ...
            <div><div className="text-[9px] sm:text-[10px] font-bold uppercase">Site 3</div><div className="text-[11px] sm:text-[13px] font-bold ">Delivery Approved</div></div>
          </div>
          <div className="bg-card border border-stone-200 rounded-lg p-3 sm:p-4 flex flex-col gap-2 rotate-[-1deg] mr-4">
            <div className="flex justify-between items-center"><span className="text-[9px] sm:text-[10px] font-bold uppercase">Monthly Spend</span><span className="text-[11px] sm:text-[13px] font-bold ">₹14.2 L</span></div>
            <div className="w-full h-1.5 bg-app rounded-full overflow-hidden"><div className="bg-app w-[65%] h-full" /></div>
          </div>
        </div>
      </div>
    )
  },
  {
    ...
    accentClass: 'bg-app ',
    tabClass: 'bg-app',
    activeTabClass: 'bg-card border-orange-200 text-primary-text border-b-2 border-b-orange-600',
    ...
    Visual: () => (
      <div className="w-full h-full min-h-[220px] sm:min-h-[300px] flex items-center justify-center p-4 sm:p-8 relative overflow-hidden">
        ...
        <div className="relative z-10 flex flex-col gap-4 sm:gap-6 w-full max-w-[280px]">
          <div className="bg-card border border-orange-200 rounded-lg p-3 sm:p-4 rotate-[2deg] flex gap-3 items-center">
             ...
             <div><div className="text-[9px] sm:text-[10px] font-bold text-orange-400 uppercase">Item Profit</div><div className="text-[11px] sm:text-[13px] font-bold text-primary-text">Pasta: 62% Margin</div></div>
          </div>
          <div className="bg-card border border-orange-200 rounded-lg p-3 sm:p-4 rotate-[-1deg] ml-6 flex flex-col gap-2">
            ...
            <button className="py-1.5 w-full rounded bg-app text-orange-600 text-[10px] font-bold border border-orange-100 uppercase">Auto-Reorder Sent</button>
          </div>
        </div>
      </div>
    )
  },
  {
    ...
    icon: Wrench,
    bgClass: 'bg-app',
    accentClass: 'bg-app text-white',
    tabClass: 'bg-app',
    activeTabClass: 'bg-card border-slate-200 text-primary-text border-b-2 border-b-slate-800',
    ...
    Visual: () => (
      <div className="w-full h-full min-h-[220px] sm:min-h-[300px] flex items-center justify-center p-4 sm:p-8 relative overflow-hidden">
        ...
        <div className="relative z-10 flex flex-col gap-4 sm:gap-5 w-full max-w-[280px]">
          <div className="bg-card border border-slate-200 rounded-lg p-3 sm:p-4 flex gap-3 items-center">
            ...
            <div className="flex-1"><div className="text-[9px] sm:text-[10px] font-bold text-slate-400 uppercase">Teak Door Quote</div><div className="flex justify-between mt-0.5"><span className="text-[11px] sm:text-[13px] font-bold text-primary-text">Cost: ₹4,200</span><span className="text-[10px] sm:text-[11px] font-bold text-emerald-600">32% Mar</span></div></div>
          </div>
          <div className="bg-card border border-slate-200 rounded-lg p-3 sm:p-4 ml-4 flex flex-col gap-2 text-[9px] sm:text-[10px]">
...
const AnimatedBeforeAfter = ({ before, after, inView }: { before: string, after: string, inView: boolean }) => {
  return (
    <div className="flex flex-col">
      <div className="text-[12px] font-bold text-muted-text line-through decoration-muted-text/50 decoration-2 mb-0.5">{before}</div>
...
const UseCasesSection = () => {
  ...
  return (
    ...
    <div ref={containerRef} className="relative h-[300vh] w-full" id="use-cases">
      <div className={`sticky top-0 h-[100dvh] w-full flex flex-col justify-center max-md:justify-start max-md:pt-[12dvh] overflow-hidden transition-colors duration-700 ease-in-out border-y border-border-main ${activeCase.bgClass}`}>

        <div className="w-full max-w-[1200px] mx-auto px-4 sm:px-6 flex flex-col gap-4">

          {/* Header */}
          <div className="text-center relative z-10 w-full">
            <span className="inline-block text-[10px] sm:text-[12px] font-bold tracking-[2.5px] uppercase text-secondary-text mb-2 sm:mb-4 px-3 sm:px-[18px] py-1 sm:py-1.5 bg-card/60 border border-border-main rounded-full backdrop-blur-md">
              See Yourself in the Story
            </span>
            <h2 className="text-[28px] sm:text-[48px] font-bold text-primary-text tracking-tight font-display mb-1">
              Real-World ROI
            </h2>
          </div>

          {/* Tab Switcher */}
          <div className="flex gap-2 max-w-[800px] mx-auto bg-card/60 p-1.5 sm:p-2 border border-border-main rounded-xl backdrop-blur-md justify-center">
            {useCases.map((uc, i) => {
          ...
          {/* Content Area */}
          <div className="grid grid-cols-1 lg:grid-cols-2 overflow-hidden bg-card border border-border-main rounded-2xl w-full max-md:flex-1 max-md:max-h-[60dvh] max-md:overflow-y-auto">

            {/* Visual Side */}
            <div className={`relative border-b lg:border-r lg:border-b-0 border-border-main transition-colors duration-700 min-h-[180px] sm:min-h-[220px] lg:h-auto ${activeCase.bgClass}`}>
            ...
            <div className="p-6 sm:p-8 lg:p-12 flex flex-col justify-center bg-card relative">
              <div key={`text-${activeIdx}`} className="animate-[fadeInRight_0.5s_ease-out]">
                <div className="mb-4 sm:mb-6">
                  <h3 className="text-[20px] sm:text-[28px] font-bold text-primary-text leading-tight mb-2 sm:mb-4 font-display">
                    {activeCase.title}
                  </h3>
                  <p className="text-[13px] sm:text-[15px] text-secondary-text leading-relaxed">
                    {activeCase.story}
                  </p>
                </div>

                {/* Before/After Metrics */}
                <div className="grid grid-cols-2 gap-3 sm:gap-4 mt-3 sm:mt-8 pt-3 sm:pt-6 border-t border-border-main/50">
                  {activeCase.metrics.map((m, idx) => (
                    <div key={idx} className="flex gap-2 sm:gap-3 items-start">
                      <div className="shrink-0 w-6 h-6 sm:w-8 sm:h-8 rounded bg-surface border border-border-main flex items-center justify-center text-muted-text mt-0.5 sm:mt-1">
                        <m.icon size={12} className="sm:w-3.5 sm:h-3.5" />
                      </div>
                      <div className="min-w-0">
                        <div className="text-[9px] sm:text-[10px] font-bold uppercase tracking-wide text-muted-text mb-0.5 truncate">{m.label}</div>
