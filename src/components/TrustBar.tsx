...
const AnimatedNumber = ({ value, suffix, inView }: { value: string; suffix: string; inView: boolean }) => {
  ...
  return (
    <span className="font-display text-[52px] max-md:text-[40px] font-bold text-black leading-none mb-2.5 tracking-tight">
      {display}<span className="text-[22px] text-neutral-400 ml-1 font-semibold">{suffix}</span>
    </span>
  )
}
...
const TrustBar = () => {
  ...
  return (
    <section className="relative py-24 bg-[#fafafa] border-t border-b border-neutral-200 overflow-hidden" ref={ref}>
      ...
      <div className="max-w-[1200px] mx-auto px-6 grid grid-cols-3 max-md:grid-cols-1 gap-6 relative z-[1]">
        {metrics.map((m, i) => (
          <div
            key={i}
            className={`flex flex-col items-center text-center p-10 max-md:p-7 bg-white border border-neutral-200 rounded-2xl transition-all duration-700 ease-[cubic-bezier(0.16,1,0.3,1)] hover:-translate-y-1 ${
              inView ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-[30px]'
            }`}
            style={{ transitionDelay: `${i * 0.15}s` }}
          >
            <span className="flex items-center justify-center w-14 h-14 bg-neutral-50 border border-neutral-200 rounded-[14px] mb-4 text-black">
              <m.Icon size={28} strokeWidth={1.8} />
            </span>
            <AnimatedNumber value={m.value} suffix={m.suffix} inView={inView} />
            <span className="text-[17px] font-semibold text-neutral-900 mb-1.5">{m.label}</span>
            <span className="text-[14px] text-neutral-500 leading-relaxed max-w-[220px]">{m.desc}</span>
          </div>
        ))}
      </div>
    </section>
