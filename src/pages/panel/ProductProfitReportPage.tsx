...
import { Search, RotateCw, ArrowUpDown, ArrowUp, ArrowDown, HelpCircle, Package2 } from 'lucide-react'
import Skeleton from 'react-loading-skeleton'
import { PageHeader } from '../../components/common/PageHeader'
import { SiteFilterSingle } from '../../components/filters/SiteFilterSingle'
import { DateRangePicker } from '../../components/common/DateRangePicker'
import { ReportService, type ProductProfitReportItem } from '../../services/reportService'
...
export default function ProductProfitReportPage() {
  ...
  const [data, setData] = useState<ProductProfitReportItem[]>([])
  ...
  const fetchReport = useCallback(async () => {
    ...
    setLoading(true)
    try {
      const res = await ReportService.fetchProductWiseProfitReport({
      ...
      let arrayData: ProductProfitReportItem[] = []
      if (rawData) {
      ...
      console.error('Failed to fetch profit report:', e)
      setData([])
    } finally {
      setLoading(false)
    }
  ...
  return (
    <div className="p-4 md:p-6 max-w-[1400px] mx-auto w-full flex flex-col gap-6 overflow-y-auto h-full">
      ...
      <PageHeader
        ...
        description="Detailed insights into transaction volumes, purchase expenses, and net profit margins per product"
      />
            ...
            <thead className="bg-table-head sticky top-0 z-10 border-b border-border-main">
              <tr>
                ...
                <th
                  ...
                  className="px-4 py-3.5 text-[10px] font-black text-muted-text uppercase tracking-wider text-right w-[15%] cursor-pointer select-none group"
                >
                ...
                <th
                  ...
                  className="px-5 py-3.5 text-[10px] font-black text-muted-text uppercase tracking-wider text-right w-[18%] cursor-pointer select-none group"
                >
                ...
                <tr>
                  <td colSpan={5} className="py-20 text-center">
                    <div className="flex flex-col items-center justify-center gap-2">
                      <Package2 size={36} className="text-muted-text opacity-40 animate-pulse" />
                      <p className="text-[13px] font-bold text-muted-text">
                        No product profit metrics found
                      </p>
                        ...
                        <div className="flex items-center gap-3">
                          {item.imageUrl ? (
                            <img
                              ...
                              className="w-8 h-8 rounded-lg object-cover bg-surface border border-border-main shadow-xs shrink-0"
                            />
                          ) : (
                          ...
                          <div className="min-w-0">
                            ...
                            <span className="block text-[10px] font-bold text-muted-text uppercase tracking-widest mt-0.5">
                              Unit: {item.unit}
                            </span>
                          </div>
                        </div>
